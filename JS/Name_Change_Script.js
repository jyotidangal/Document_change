window.onload = () => {
  ...
  document.getElementById("meeting-form").addEventListener("submit", (event) => {
    ...
    document.querySelectorAll("[id^='r_company_current_english']").forEach((el) => {
      el.innerText = companyCurrentEnglish
    })
  
    document.querySelectorAll("[id^='r_company_new_nepali']").forEach((el) => {
      el.innerText = companyNewNepali
    })
  
    document.querySelectorAll("[id^='r_company_new_english']").forEach((el) => {
      el.innerText = companyNewEnglish
    })
  ...
  document.getElementById("downloadPdf").addEventListener("click", async () => {
    try {
      ...
      document.querySelectorAll("[id^='r_company_current_english']").forEach((el) => {
        if (!el.innerText) el.innerText = companyCurrentEnglish
      })
      document.querySelectorAll("[id^='r_company_new_nepali']").forEach((el) => {
        if (!el.innerText) el.innerText = companyNewNepali
      })
      document.querySelectorAll("[id^='r_company_new_english']").forEach((el) => {
        if (!el.innerText) el.innerText = companyNewEnglish
      })
      document.querySelectorAll("[id^='r_chairperson']").forEach((el) => {
        ...
        allElements.forEach((el) => {
          ...
          if (el.id && el.id.startsWith("r_")) {
            if (el.id.includes("company_current_nepali") && !el.innerText) {
              el.innerText = companyCurrentNepali
            } else if (el.id.includes("company_current_english") && !el.innerText) {
              el.innerText = companyCurrentEnglish
            } else if (el.id.includes("company_new_nepali") && !el.innerText) {
              el.innerText = companyNewNepali
            } else if (el.id.includes("company_new_english") && !el.innerText) {
              el.innerText = companyNewEnglish
      ...
      // Save the PDF
      doc.save(`Name_Change_${document.getElementById("meeting_date").value.replace(/\//g, "-")}.pdf`)
